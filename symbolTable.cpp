...
bool SymbolTable::Scope::insert(string symbol, Node * node) {
   if( symbols.find(symbol) == symbols.end() ) {
      ...
      if( debugFlag ) {
         if ( debugFlag ) printf("DEBUG (Scope): Inserted \"%s\" in \"%s\" scope but the symbol is already there!\n", symbol.c_str(), name.c_str());
         return false;
      }
   }
   return false;
...
Node * SymbolTable::Scope::lookup(string symbol) {
   if( symbols.find(symbol) != symbols.end() )
   {
      if( debugFlag ) printf("DEBUG (Scope): Found \"%s\" symbol in \"%s\" scope.\n", symbol.c_str(), name.c_str());
      return symbols[symbol];
   }
   else {
      if( debugFlag ) printf("DEBUG (Scope): Did NOT find \"%s\" symbol in \"%s\" scope.\n", symbol.c_str(), name.c_str());
      return NULL;
   }
}
...
void SymbolTable::Scope::checkIfDeclared(string symbol) {
   if( symbols.find(symbol) == symbols.end() ) {
...
void SymbolTable::enter(string name) {
   if( debugFlag ) printf("DEBUG (SymbolTable): Entered scope \"%s\".\n", name.c_str());
   stack.push_back(new Scope(name));
}

void SymbolTable::leave() {
    if ( debugFlag ) printf("DEBUG (SymbolTable): Leaving scope \"%s\".\n", (stack.back()->scopeName()).c_str());
    if ( stack.size() > 1 ) {
        delete stack.back();
        stack.pop_back();
    }
...
bool SymbolTable::insertGlobal(string symbol, Node * node) {
    if ( debugFlag ) {
         printf("DEBUG (Scope): Insert the Global symbol \"%s\"", symbol.c_str());
        if( node == NULL ) printf(" WARNING: The inserted pointer is NULL!");
        printf("\n");
    }
...
Node * SymbolTable::lookup(string symbol) {
   ...
   if ( debugFlag ) {
       ...
       if ( data ) printf("found in the \"%s\" scope.\n", name.c_str());
       else printf("is not in any of the scopes!\n");
   }
   return data;
} 
...
Node * SymbolTable::lookupGlobal(string symbol) {
   ...
   if ( debugFlag ) printf("DEBUG (SymbolTable): Symbol \"%s\" in the Globals and %s.\n", symbol.c_str(), (data ? "found it" : "did NOT find it"));
   return data;
}
...
void SymbolTable::checkIfDeclared(string symbol) {
   for ( vector<Scope *>::reverse_iterator it = stack.rbegin(); it != stack.rend(); it++ ) {
      (*it)->checkIfDeclared(symbol);
   }   
}
// #####################################################################################################
// #                                     SYMBOLTABLE CLASS FUNCTIONS                                   #
...
Node * SymbolTable::lookupScopeName(string symbol, string scopeName) {
   for ( vector<Scope *>::reverse_iterator it = stack.rbegin(); it != stack.rend(); it++ )
   {
