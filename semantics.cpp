...
bool checkInit = false;                // Conditional variable to check variable for initialization.
bool inLoop = false;                   // Are we in a loop?
      ...
      if( AST->nodeType == FuncNT ) {
         if( string(AST->literal).compare("main") == 0 ) {
            ...
            if( checkIfDeclaredAlreadyGlobal(AST, ST) == false ) {
               if( AST->child[0] != NULL ) {
                  if( AST->child[0]->nodeType == ParmNT || AST->child[0]->nodeType == ParmArrNT ) {
                     hasMain = false;
                  }
                  else {
                     hasMain = true;
      ...
      if( isSymbol(AST) && AST->isVisited == false ) {
         ...
         if ( AST->child[0] != NULL ) {
            if( AST->child[0]->nodeType == IdNT ) {
               ...
               AST->lineNum, AST->child[0]->literal);
               errs += 1;
            }
   ...
   if( AST != NULL ) {
      ...
      if( isSymbol(AST) ) {
         ...
         if( AST->nodeType == ParmNT || AST->nodeType == ParmArrNT ) {
            if( parentScope != NULL ) {
               parentScope->ParmList.insert(make_pair(parentScope->parmC, AST));
...
Node * processLHS(Node * node, SymbolTable * ST) {
   if( node != NULL ) {
      ...
      else if ( node->nodeType == OpNT )
      {
         ...
         return lsym; // RETURN NODE OP TO RETAIN DT! Instead of 'lsym', do 'node'
      }
   }
   return NULL;
}
