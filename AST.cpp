...
char * convertRTStr(Node * node) {
   char * tmp;
   if( node->refType == LocalRT ) tmp = (char *)"Local";
   if( node->refType == GlobalRT ) tmp = (char *)"Global";
   if( node->refType == StaticRT ) tmp = (char *)"LocalStatic";
   if( node->refType == ParamRT ) tmp = (char *)"Parameter";
   if( node->refType == NoneRT ) tmp = (char *)"None";
   return tmp;
}
