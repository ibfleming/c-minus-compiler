...
varDeclId : ID                      { $$ = createNode($1, VarNT); $$->isInit = false; }       
          | ID OBRK NUMCONST ']'    { $$ = createNode($1, VarArrNT); $$->isArray = true; $$->isInit = false; }
          ;
...
parmId : ID                         { $$ = createNode($1, ParmNT); $$->isInit = true; }
       | ID OBRK ']'                { $$ = createNode($1, ParmArrNT); $$->isArray = true; $$->isInit = true; }
       ;
...
constant : NUMCONST           { $$ = createNode($1, NumConstNT); $$->dataType = IntDT; $$->isConst = true; $$->isInit = true; }              
         ...
         | STRINGCONST        { $$ = createNode($1, StringConstNT); $$->dataType = CharDT; $$->isConst = true; $$->isArray = true;  $$->isInit = true; }          
...
int main(int argc, char *argv[]) {
   ...
   SymbolTable ST;
   rAST = createRoutineAST();
   ST = traverseRoutineAST(rAST, &ST);
   traverseAST(AST, &ST);
   checkMain(&ST);
   //printAST(AST, 0, TYPES);
   if( errs == 0 ) {
